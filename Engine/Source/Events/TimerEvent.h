...
public:
    ...
    static inline engine::memory::SharedPointer<TimerEvent> Create(const std::function<void(void)>& i_on_timer_elapsed, float i_duration, int32_t i_repeat);
    ...
    inline void SetDuration(float i_duration);
    inline int32_t GetRepeat() const;
    inline void SetRepeat(int32_t i_repeat);

private:
    TimerEvent();
    TimerEvent(const std::function<void(void)>& i_on_timer_elapsed, float i_duration, int32_t i_repeat);
