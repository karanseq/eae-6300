...
#ifdef BUILD_DEBUG
unsigned int BlockDescriptor::counter = 0;
#endif
...
BlockAllocator::BlockAllocator(void* i_memory, size_t i_block_size) : block_(nullptr),
	user_list_head_(nullptr),
	free_list_head_(nullptr),
	total_block_size_(i_block_size)
{
	block_ = static_cast<uint8_t*>(i_memory);
#ifdef BUILD_DEBUG
...
void* BlockAllocator::Alloc(const size_t i_size, const size_t i_alignment)
{
	...
	while (free_bd != nullptr)
	{
		...
		if (i_size <= free_bd->block_size)
		{
			...
			const uintptr_t			alignment_offset = (reinterpret_cast<uintptr_t>(new_block_pointer + size_of_BD_ + guardband_size)) % i_alignment;
			...
			else if ((i_size + guardband_size * 2) <= free_bd->block_size)
			{
				...
				if ((reinterpret_cast<uintptr_t>(free_bd->block_pointer + guardband_size) % i_alignment) == 0)
				{
...
#ifdef BUILD_DEBUG
			LOG("Merging %zu bytes from block-%d & block-%d", (next_bd->block_size + size_of_BD_), curr->id, next_bd->id);
#endif
...
void BlockAllocator::PrintFreeDescriptors() const
{
	...
	if (free_list_head_ != nullptr)
	{
		...
		{
			LOG("BD.id:%d size:%zu", bd->id, bd->block_size);
			++count;
		}
