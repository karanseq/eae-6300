...
inline engine::memory::SharedPointer<PhysicsObject> PhysicsObject::Create(const engine::memory::WeakPointer<engine::gameobject::GameObject>& i_game_object, float i_mass, float i_drag, bool i_is_collidable)
{
	return engine::memory::SharedPointer<PhysicsObject>(new PhysicsObject(i_game_object, i_mass, i_drag, i_is_collidable));
}
